...
class UserGoalViewset(viewsets.ModelViewSet):
    ...
    def create_user_goal(self, request):
        try:
            user = request.user
            friend = request.data['friend_name']
            
            UserGoalSerializer = self.serializer_class(data=request.data)
            if UserGoalSerializer.is_valid(raise_exception=True):
                print(UserGoalSerializer.validated_data)
                friend_array = friend.split()
                friend_user = User.objects.filter(first_name=friend_array[0], last_name=friend_array[1])
                if not friend_user.exists():
                    return Response({"message": "User does not exist"}, status=status.HTTP_404_NOT_FOUND)
                friend_user = friend_user.first()
                if Friend.objects.filter(friend__id=friend_user.id, user__id=user.id).exists():
            ...
            return Response(UserGoalSerializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_users_goals(self, request):
        ...
        if not user_goals:
            return Response({"message": "No user goals found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def get_a_user_goals(self, request):
        try:
            ...
            return Response({"message": "User goals not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def update_a_user_goal(self, request):
        try:
            ...
            serializer = self.serializer_class(user_goal, data=request.data, partial=True)
            ...
            return Response({"message": "Error updating goal"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def delete_a_user_goal(self, request):
        try:
            ...
            return Response({"message": "Error deleting goal"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
